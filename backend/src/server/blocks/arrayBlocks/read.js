...
export async function getArrayBlockWithNestedContent(block_id) {
  ...
  const arrayBlock = await db.query.arrayBlocks.findFirst({
    where: (arrayBlocks, { eq }) => eq(arrayBlocks.block_id, block_id),
  });
    ...
    items.map(async (item) => {
      if (item.item_type === "text_field") {
        const field = await db.query.textFields.findFirst({
          where: (textFields, { eq }) => eq(textFields.field_id, item.reference_id),
        });
        ...
        const field = await db.query.textAreaFields.findFirst({
          where: (textAreaFields, { eq }) => eq(textAreaFields.field_id, item.reference_id),
        });
        return { item_type: "textarea_field", textarea_field: field };
      } else if (item.item_type === "block") {
        const nestedContent = await getArrayBlockWithNestedContent(item.reference_id);
        return { item_type: "block", block: nestedContent };
      } else if (item.item_type === "array_block") {
        // Handle array blocks specifically
        const nestedArrayBlock = await getArrayBlockWithNestedContent(item.reference_id);
        return { item_type: "array_block", array_block: nestedArrayBlock };
      }
      return null;
    })
  );
  
  const childArrayBlocks = await db.query.arrayBlocks.findMany({
    where: (arrayBlocks, { eq }) => eq(arrayBlocks.reference_id, arrayBlock.block_id),
  });
  
  const processedArrayBlocks = await Promise.all(
    childArrayBlocks.map(async (childBlock) => {
      const nestedContent = await getArrayBlockWithNestedContent(childBlock.block_id);
      return { 
        item_type: "array_block", 
        array_block: nestedContent 
      };
    })
  );
  
  const allProcessedItems = [...processedItems, ...processedArrayBlocks].filter(Boolean);
  
  return {
    ...arrayBlock,
    block_items: allProcessedItems
  };
}
