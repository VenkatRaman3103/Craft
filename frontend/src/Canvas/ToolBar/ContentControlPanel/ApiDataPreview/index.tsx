...
const ApiDataPreview: React.FC<DataPreviewProps> = ({
    data,
    maxDepth = 10,
    showCopyButton = true,
    onKeySelect,
...
const StructuredView: React.FC<{
    ...
    onKeySelect?: (keyPath: string) => void;
    selectedKey?: string;
}> = ({ data, maxDepth, onCopy, copiedPath, onKeySelect, selectedKey }) => {
    return (
...
> = ({
    ...
    const handleKeyClick = (keyPath: string) => {
        if (onKeySelect) {
            onKeySelect(keyPath);
        }
    };

    const isKeySelected = (keyPath: string): boolean => {
        return selectedKey === keyPath;
    };
    ...
    const renderPrimitiveValue = (
        ...
        const isSelected = isKeySelected(currentPath);

        return (
            <div
                key={currentPath}
                className={`data-row data-row--${dataType} ${isSelected ? "data-row--selected" : ""}`}
            >
                <div className="data-row__key">
                    ...
                    <span
                        className="data-row__path data-row__path--clickable"
                        onClick={() => handleKeyClick(currentPath)}
    ...
    const renderComplexValue = (
        ...
        return (
            <div key={currentPath} className="data-group">
                <div
                    className={`data-row data-row--${dataType} data-row--expandable ${isSelected ? "data-row--selected" : ""}`}
                >
                    <div className="data-row__key">
                        <button
